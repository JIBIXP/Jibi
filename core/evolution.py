...
def _niveau_autonomie() -> str:
    try:
        from core.autonomie import get_niveau
        return get_niveau()
    except Exception:
        return "assiste"
...
def protege(fichier: str) -> tuple:
    ...
    # Toujours interdits, quel que soit le niveau d'autonomie.
    if p.parts and p.parts[0] in (".git", "workspace"):
        return True, f"Dossier protégé : {p.parts[0]}/"
    if p.name.startswith(".env"):
        return True, "Fichier protégé : secrets (.env)"
    if p.suffix != ".py":
        return True, "Seuls les .py sont modifiables automatiquement"
    if _niveau_autonomie() != "autonome":
        # Modes assisté / semi_auto : protections historiques.
        if p.parts and p.parts[0] in DOSSIERS_PROTEGES:
            return True, f"Dossier protégé : {p.parts[0]}/ (passe en mode autonome pour le modifier)"
        if p.name in FICHIERS_PROTEGES:
            return True, f"Fichier protégé : {p.name}"
    else:
        # Mode autonome : core/ et self_improvement/ modifiables (avec
        # backup + tests + rollback). updater/ reste sanctuarisé pour ne
        # jamais casser le mécanisme de mise à jour lui-même.
        if p.parts and p.parts[0] == "updater":
            return True, "Dossier protégé : updater/ (mécanisme de mise à jour)"
    return False, ""
...
def appliquer(pid: str, confirmation: str, interne: bool = False) -> dict:
    """Applique une proposition. `interne=True` = application par le moteur
    d'autonomie lui-même : la phrase humaine est remplacée par une double
    vérification (niveau == autonome + proposition à faible risque)."""
    p = charger(pid)
    if not p:
        return {"succes": False, "erreur": f"Proposition {pid} introuvable"}
    if interne:
        if _niveau_autonomie() != "autonome":
            return {"succes": False, "erreur": "Application interne refusée : mode autonome requis"}
        try:
            from core.autonomie import evaluer_risque
            verdict = evaluer_risque(p)
            if not verdict.get("faible_risque"):
                return {"succes": False, "erreur": f"Application interne refusée : {verdict.get('raison')}"}
        except ImportError:
            return {"succes": False, "erreur": "Moteur d'autonomie indisponible"}
    elif confirmation.replace("’", "'").strip().upper() != f"J'AUTORISE {pid}".upper():
    ...
    try:
        ...
        if p["fichier"].startswith("tools/"):
            ...
            if mod in sys.modules:
                module_recharge = importlib.reload(sys.modules[mod])
            else:
                module_recharge = importlib.import_module(mod)
            # FIX : sans ça, TOOLS_REGISTRY gardait les ANCIENNES fonctions et
            # l'agent continuait d'exécuter l'ancien code jusqu'au redémarrage.
            _rafraichir_registre_outils(mod, module_recharge)
...
def _rafraichir_registre_outils(nom_module: str, module_recharge) -> int:
    """Remplace dans TOOLS_REGISTRY les fonctions de `nom_module` par celles
    du module rechargé (mêmes noms, mêmes schémas, nouveau code)."""
    try:
        from tools.tool_registry import TOOLS_REGISTRY
    except Exception:
        return 0
    maj = 0
    for _nom, spec in list((TOOLS_REGISTRY or {}).items()):
        fn = (spec or {}).get("function")
        if getattr(fn, "__module__", None) != nom_module:
            continue
        nouvelle = getattr(module_recharge, getattr(fn, "__name__", ""), None)
        if callable(nouvelle) and nouvelle is not fn:
            spec["function"] = nouvelle
            maj += 1
    if maj:
        _log(f"Registre outils rafraîchi : {maj} fonction(s) de {nom_module}")
    return maj
